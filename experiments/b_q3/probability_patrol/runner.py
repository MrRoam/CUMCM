...
def output_path(path):
    """新产物只能进入 outputs；历史输入与源码不可作为输出目标。"""
    path = Path(path).resolve()
    base = (ROOT / 'outputs/experiments/b_q3').resolve()
    if base not in path.parents:
        raise ValueError('输出必须位于 outputs/experiments/b_q3/<run-id>/ 内')
    return path


def new_run(path):
    """每次创建独立批次，并保存完整活动源码以恢复未提交版本。"""
    import subprocess
    out = output_path(path)
    out.mkdir(parents=True, exist_ok=False)
    files = set()
    for folder in ('b_q3', 'b_adaptive_q3', 'b_benchmark_scale', 'b_overnight', 'b_oracle_q3'):
        for source in (ROOT / 'experiments' / folder).rglob('*.py'):
            relative = source.relative_to(ROOT)
            if not any(part in ('runs', 'source', '__pycache__', 'Q3_失败方案交接包') for part in relative.parts):
                files.add(source)
    hashes = {}
    ...
    command[0] = 'python'
    if entry.is_relative_to(ROOT):
        command[1] = entry.relative_to(ROOT).as_posix()
    dump(out / 'execution.json', dict(
        status='running', command=command, python=platform.python_version(),
        created_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
        base_commit=subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
        source_sha256=hashes, source_snapshot='source',
        note='源码快照是本次实际版本；base_commit 不代表没有未提交改动。'))
    return out


def finish_run(out, expected):
    results = [json.loads(p.read_text(encoding='utf-8')) for p in Path(out).rglob('result.json')]
    success = len(results) == expected and all(r.get('success') for r in results)
    path = Path(out) / 'execution.json'
    record = json.loads(path.read_text(encoding='utf-8'))
    record.update(status='completed' if success else 'failed', expected_results=expected,
                  actual_results=len(results))
    dump(path, record)
    if not success:
        raise RuntimeError('批次未全清或结果缺失，请查看本次运行的失败记录')
...
def read_cases(path):
    ...
    for case in cases:
        if not case.name or case.name in ('.', '..') or any(c in case.name for c in '/\\:'):
            raise ValueError('案例名称必须是单个安全目录名')
