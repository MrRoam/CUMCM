...
def main():
    ...
    p.add_argument('--workers',type=int,default=3);args=p.parse_args();out=a.base.new_run(args.out)
    if (out/'cases.json').exists():cases=[a.Scenario.from_dict(c) for c in json.loads((out/'cases.json').read_text(encoding='utf-8'))]
