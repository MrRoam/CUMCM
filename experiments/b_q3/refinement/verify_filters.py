...
def main(out):
    ...
    for meta_path in sorted(out.glob('*/f1/metadata.json')):
        ...
        actions=[json.loads(s) for s in (directory/'actions.jsonl').read_text(encoding='utf-8').splitlines()]
        original=[json.loads(s) for s in (directory.parent/'current/actions.jsonl').read_text(encoding='utf-8').splitlines()]
        records=json.loads(meta_path.read_text(encoding='utf-8'))['skipped']
        by_prefix={}
