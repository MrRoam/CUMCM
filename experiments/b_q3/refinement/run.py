...
def selection_for(case, directory):
    ...
    if cache.exists():
        selection=json.loads(cache.read_text(encoding='utf-8'))
        ...
        raw=json.loads((a.HERE/'demonstration/replay.json').read_text(encoding='utf-8'))
...
def summarize(out,modes):
    ...
    for c in cases:
        ...
        for mode in modes:
            ...
            if path.exists():row[mode]=json.loads(path.read_text(encoding='utf-8'))
        rows.append(row)
...
def main():
    ...
    args=p.parse_args()
    out=a.base.new_run(args.out)
    if args.stage=='development':
        cases=[a.Scenario.from_dict(json.loads((a.HERE/'demonstration/case.json').read_text(encoding='utf-8')))]
    ...
    summarize(out,args.modes)
    a.base.finish_run(out, len(cases)*len(args.modes))
