...
import traceback
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "probability_patrol"))
import runner as runtime
...
def main():
    ...
    out = runtime.new_run(ROOT / 'outputs/experiments/b_q3/decision_pilot' / args.output)
    ...
    record.update(status='failed' if failures else 'completed', expected_cases=len(cases), completed_cases=len(diagnoses))
