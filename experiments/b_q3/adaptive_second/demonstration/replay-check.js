...
(() => {
  const root=document.getElementById('patrol-walkthrough');
  const data=JSON.parse(document.getElementById('patrol-walkthrough-data').textContent);
  ...
  function draw(){
    ...
    const defs=svg('defs'),clip=svg('clipPath',{id:'patrol-domain'}),viewport=svg('clipPath',{id:'patrol-viewport'});
    ...
    const scene=svg('g',{'clip-path':'url(#patrol-viewport)'});s.append(scene);
    ...
    const clipped=svg('g',{'clip-path':'url(#patrol-domain)'});scene.append(clipped);
    if(chosen){
      const histories=data.actions.slice(0,g.end).filter(a=>a.channel===chosen&&a.kind==='measure');
      ...
      const bearings=histories.filter(a=>a.result==='direction').slice(-3);
      bearings.forEach(a=>{const theta=a.bearing*Math.PI/180,q=a.position;const p=[q,[q[0]+1500*Math.cos(theta-Math.PI/180),q[1]+1500*Math.sin(theta-Math.PI/180)],[q[0]+1500*Math.cos(theta+Math.PI/180),q[1]+1500*Math.sin(theta+Math.PI/180)]];clipped.append(svg('polygon',{points:p.map(v=>`${X(v[0])},${Y(v[1])}`).join(' '),fill:color.range,'fill-opacity':.07,stroke:color.range,'stroke-opacity':.3,'stroke-width':.8}));});
    }
  ...
  function render(autoFocus=true){
    ...
    ui.focus.textContent=c?.status==='found'?`关注 C${chosen}：可能区域包围半径 ${fmt(c.radius)}米；填色是保守几何范围，不是概率热图。`:
