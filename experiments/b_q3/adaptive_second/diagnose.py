...
def analyze(out):
    out=Path(out)
    cases=[a.Scenario.from_dict(x) for x in json.loads((out/'cases.json').read_text(encoding='utf-8'))]
    rng=np.random.default_rng(290912)
    rows=[]
    action_count=0
    for case in cases:
        directory=out/case.name
        sel=json.loads((directory/'selection.json').read_text(encoding='utf-8'))
        oracle=json.loads((directory/'oracle_diagnostic.json').read_text(encoding='utf-8'))
        row={'case':case.name,'layout':case.layout}
        for mode in ('fixed','position','joint','baseline'):
            action_count+=replay(directory/mode,case)
            r=json.loads((directory/mode/'result.json').read_text(encoding='utf-8'))
